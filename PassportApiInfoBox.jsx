...
const PassportApiInfoBox = ({ apiKey, serviceName, apiUrl }) => {
  return (
          ...
          {apiKey ? (
            <>
              <p className="text-xs text-green-600">
                API Key is configured. The tool will attempt to use the API ({apiUrl || 'configured endpoint'}) for processing.
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                Ensure the API key is valid and the API endpoint is correct.
              </p>
            </>
          ) : (
...
export default PassportApiInfoBox;
